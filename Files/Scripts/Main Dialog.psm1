...
function CreateLanguagesContent() {
    ...
    if ($GameType.mode -eq "Ocarina of Time") {
        ...
        $Languages.PauseScreen            = CreateReduxCheckBox    -Column 1 -Row 3 -AddTo $Languages.TextBox  -Text "MM Pause Screen"         -ToolTip $ToolTip -Info "Replaces the Pause Screen textures to be styled like Majora's Mask" -Name "PauseScreen"
...
function UnlockLanguageContent() {
    ...
    # English options
    $Languages.TextRestore.Enabled = $Languages.TextFemalePronouns.Enabled = $Languages.TextDialogueColors.Enabled = $Languages.TextUnisizeTunics.Enabled = $Languages.PauseScreen.Enabled = $Languages[0].checked
