function PatchOptionsSuperMetroid() {
    ...
    if (IsChecked -Elem $Redux.Widescreen)           { ApplyPatch -File $GetROM.decomp -Patch "\Compressed\widescreen.ips" }
    if (IsChecked -Elem $Redux.RedesignedSamus)      { ApplyPatch -File $GetROM.decomp -Patch "\Compressed\redesigned_samus.ips" }

    if (IsChecked -Elem $Redux.HeavyPhysics)         { ApplyPatch -File $GetROM.decomp -Patch "\Compressed\heavy_physics.ips" }
    if (IsChecked -Elem $Redux.SaveStationsRefill)   { ApplyPatch -File $GetROM.decomp -Patch "\Compressed\save_stations_refill_everything.ips" }
    if (IsChecked -Elem $Redux.SkipCeres)            { ApplyPatch -File $GetROM.decomp -Patch "\Compressed\skip_ceres.ips" }
...
function PatchReduxSuperMetroid() {
    ...
    if (IsChecked -Elem $Redux.FixedUnlockedDoors)   { ApplyPatch -File $GetROM.decomp -Patch "\Compressed\fixed_tourian_unlocked_doors.ips" }
...
function CreateOptionsSuperMetroid() {
    ...
    $Redux.Widescreen.enabled = !$IsWiiVC
...
function CreateTabMainSuperMetroid() {
    ...
    # GAMEPLAY #
    CreateReduxGroup -Height 2 -Text "Gameplay"
